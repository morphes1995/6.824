...
func (kv *ShardKV) myInfo() string {
	...
	return fmt.Sprintf("%v gid %v isLeader %v", kv.me, kv.gid, isLeader)
}
...
func (kv *ShardKV) applyConfigurationChange(conf ConfigurationOp, commitIndex int) {
	...
	if conf.Config.Num != kv.config.Num+1 {
		...
		for shardId, shardData := range conf.Data {
			for k, v := range shardData {
				oldVal, _ := kv.data[shardId][k]
				DPrintf("%s change val of key in shard %v from %v to %v, src gid %v, conf num %v -> %v", kv.myInfo(), shardId, oldVal, v, conf.Config.Shards[shardId], kv.config.Num, conf.Config.Num)
				kv.data[shardId][k] = v
			}
		}
